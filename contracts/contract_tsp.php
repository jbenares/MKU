...
$aRequest	= array('b','tsp_id','keyword');
$aHeader 	= 	array(
					'employeeID','con_date','effectivity_date','project_id','base_rate','allowance','others','position'
				);
...
if($aVal['b'] == "Submit"){
	if(empty($aVal['tsp_id'])){
		...
		$aVal['tsp_id'] = mysql_insert_id();
		mysql_query("
			update contracts_tsp set user_id = '$user_id' where tsp_id = '$aVal[tsp_id]'
		") or die(mysql_error());
...
if($aVal['tsp_id'] && $aVal['b'] != "Search"){
	$result = mysql_query("
		select * from contracts_tsp where tsp_id = '$aVal[tsp_id]'
	") or die(mysql_error());
...
<form enctype='multipart/form-data' method="post" action="" id="newareaform" >
	...
	<?php if($aVal['b'] == "Search"): ?>
        ...
        $sql = "
            select
				concat(e.employee_lname,', ',e.employee_fname) as employee, tsp_id, effectivity_date
			from
		...
		if( $_REQUEST['search_employeeID'] ) 	$sql .= " and ct.employeeID = '$_REQUEST[search_employeeID]'";
        ...
        $pagination	= $pager->renderFullNav("$view&b=Search&keyword=$aVal[keyword]&search_employeeID=$_REQUEST[search_employeeID]&search_emp0loyee=$_REQUEST[search_employee]&search_date=$_REQUEST[search_con_date]");
        ?>
       ...
       <table width="100%" align="left" style="text-align:left;" class="display_table">
        	<!--<thead> -->
                <tr>				
                    <td width="20">#</td>
                    <td width="20"></td>
                    <td style="text-align:left;">TSP-COE #</td>
            ...
            while($r=mysql_fetch_assoc($rs)) {
                ...
                echo '<td width="15"><a href="admin.php?view='.$view.'&tsp_id='.$r['tsp_id'].'" title="Show Details"><img src="images/edit.gif" border="0"></a></td>';
				echo '<td>'.str_pad($r['tsp_id'],5,0,STR_PAD_LEFT).'</td>';	
    ...
    <div class=form_layout_productmaster>
        ...
        <div class="module_actions">
            <table class="table-form">
                <input type="hidden" name="tsp_id" value="<?=$aVal['tsp_id']?>" />
        ...
        <?php endif; ?>
       	<div class="module_actions">
            ...
            <?php if($b!="Print Preview" && !empty($aVal['tsp_id'])){ ?>
        ...
        <?php 
		if($b == "Print Preview" && $aVal['tsp_id']){ 
			echo "	<iframe id='JOframe' name='JOframe' frameborder='0' src='contracts/print_contract_tsp.php?id=$aVal[tsp_id]' width='100%' height='500'>
			</iframe>";    
		} 
		?>
    </div>
