...
<div class="container">	    
    ...
    <div style="position:absolute; top:1.6cm; left:4.3cm; line-height:20px;">
    	CV#<?=str_pad($cv_no,5,0,STR_PAD_LEFT)?><br />
        <?=$cv_date?><br />
        <?=number_format($_cash_amount,2,'.',',')?><br />
        <?=$supplier?>
		<?php if($bdo) echo "<br> CHECK # : $check_no"; ?>
    </div>
    ...
    <div style="text-align:center;position:absolute; top:5.5cm; left:4.3cm;">
         <div style="border-bottom:1px solid #000;width:200px;"></div>
        <span>Received by:</span><br/>
        <span style="font-size:9pt;">(Print name and Sign)</span>
    </div>
    <div style="text-align:center;position:absolute; top:5.5cm; left:11.3cm;">
         <div style="border-bottom:1px solid #000;width:200px;"></div>
        <span>Date</span>
    </div>
    ...
    <?php if($bdo) echo "<div style='position:absolute; top:6.5cm; right:4cm;'>Received by</div>"  ?>
    ...
    <div style="position:absolute; top:12.5cm; left:2cm; width:100%; font-size:10px;">
    	<table style="width:80%; border-collapse:collapse;" class="entry">
            ...
            <?php } else { ?>
				...
				$vat_amount = $vatable * ($vat/100);
				$tax_amount = $vatable * ($wtax/100);
				$cash_amount = $total - $chargable_amount - $retention_amount - $tax_amount - $rmy_amount;
    ...
    <div style="position:absolute; top:20.3cm; left:3.7cm; width:90%; line-height:20px;font-weight:bold; font-size:10px;" >
    	<span style="position:absolute;top:0.5cm;"><?=$supplier?></span> <br /><br />
        <span style="position:absolute;top:1.3cm;"><?=$words?>ONLY</span> <br />
       <span style="position:absolute;top:2.5cm;left:5.5cm;"> CV#<?=str_pad($cv_no,5,0,STR_PAD_LEFT)?></span>
    </div>
    
    <div style="position:absolute; top:19.9cm; right:6cm; line-height:25px;font-weight:bold; font-size:10px;">
    	<span style="position:absolute;top:-0.1cm;"><?=$check_date?></span>
    	<br />
    	<span style="position:absolute;top:1cm;"><b style="font-size:16px;"><?=number_format($_cash_amount,2,'.',',')?></b></span>
    </div>
	</div>
