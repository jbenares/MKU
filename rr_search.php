...
<?php
	...
	if($_SESSION[userID] == '20160719-110150' || $_SESSION[userID] == '20200311-050946' || $_SESSION[userID] == '20170830-120801' || $_SESSION[userID] == '20200319-055723'){
		$old_id_apv = $_REQUEST['old_id_apv'];
	}
	...
	function hasAPV($rr_header_id){
		$result = mysql_query("select
					*
					...
					h.`status` != 'C' and
					d.rr_id = '$rr_header_id'") or die(mysql_error());
	...
	if($b=='Generate APV') {
		...
		$sql = mysql_query("select *
							...
							apv_header as h
							where
							apv_header_id = '$old_id_apv'") or die(mysql_error());	
							
		$count = mysql_num_rows($sql);
		
		if($count > 0){
			$msg = "APV Already Exist";
...
<div class=form_layout>
    ...
    <div class="module_actions">       
        ...
        <div style="display:inline-block;">
		 ...
		 <div style="display:inline-block;">
        	DR/Invoice #: <br />
            <input type='text' name='search_inv_no' class='textbox3' value='<?=$_REQUEST['search_inv_no']?>'>            
        </div> 
		...
		<div style="display:inline-block;">
        	APV Header ID <br />
            <input type="text" class="textbox" name="old_id_apv" />
        </div>
		<?php } ?>
        ...
        <table id="my_table" cellspacing="2" cellpadding="5" width="100%" align="center" class="display_table">
			...
			if($_REQUEST['search_inv_no']){
				$sql = "select
							*
							...
							h.`status` != 'C' and
							d.invoice like '%$search_inv_no%'
							group by h.rr_header_id
							order by
							h.date asc
                    ";		
            ...
            <?php								
                while($r=mysql_fetch_assoc($rs)) {
					...
					$disable = (hasAPV($r['rr_header_id']))?"disabled='disabled'":"";
