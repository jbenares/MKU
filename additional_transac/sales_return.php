...
function insertTransactionToDB($form_data){
    ...
    if(empty($form_data['sales_return_header_id'])){
        $sql = " insert into  sales_return_header ";
    } else {
        $sql = " update sales_return_header ";
    }
    ...
    if(empty($form_data['sales_return_header_id'])){ #insert
        $sql .= "
    ...
    if(!empty($form_data['sales_return_header_id'])){
        $sql .= "
            where sales_return_header_id = '$form_data[sales_return_header_id]'
        ";
    }

    mysql_query($sql) or die(mysql_error());
    $sales_return_header_id = $_REQUEST['sales_return_header_id'] = (!empty($form_data['sales_return_header_id'])) ? $form_data['sales_return_header_id'] : mysql_insert_id(); 

    return $sales_return_header_id;  
}
...
function addProduct(){
    ...
    $sql = "
        insert into
            sales_return_detail
        set
            sales_return_header_id = '$_REQUEST[sales_return_header_id]',
    ...
    if( count($arr) ){
        ...
        foreach ($arr as $r) {
            ...
            echo "
                <tr>
                    <td>$i</td>
                    <td><a class='trash' href='admin.php?view=$GLOBALS[view]&sales_return_header_id=$id&b=rd&id=$r[sales_return_detail_id]' onclick='return approve_confirm();'><img src='images/trash.gif'></a></td>
                    <td>$r[stock]</td>
...
if( $_REQUEST['b'] == "Submit" ){
    ...
    if( $_REQUEST['date'] ){
        ...
        $sales_return_header_id  =  $_REQUEST['sales_return_header_id'] = insertTransactionToDB($_REQUEST);            
    ...
    mysql_query("
        update sales_return_header set status = 'C' where sales_return_header_id = '$_REQUEST[sales_return_header_id]'
    ") or die(mysql_error());
    ...
    $msg = addRawMaterials();
    computeExcessQuantity($_REQUEST['sales_return_header_id']);
    ...
    $sql = "
        ...
        set
            sales_return_void = '1'
        where
            sales_return_detail_id = '$_REQUEST[id]'
    ";
    DB::conn()->query($sql) or die(DB::conn()->error);    
}


if($_REQUEST['sales_return_header_id'] && $_REQUEST['b'] != "Search"){
	$result = mysql_query("
		select 
            *
        from 
            sales_return_header        
        where sales_return_header_id = '$_REQUEST[sales_return_header_id]'
	") or die(mysql_error());
...
<form enctype='multipart/form-data' method="post" action="" id="form" >
	...
	<?php if($_REQUEST['b'] == "Search"): ?>
        ...
        <table width="100%" align="left" style="text-align:left;" class="table-css">
            ...
            while($r=mysql_fetch_assoc($rs)) {
                ...
                echo '<td width="15"><a href="admin.php?view='.$view.'&sales_return_header_id='.$r['sales_return_header_id'].'" title="Show Details"><img src="images/edit.gif" border="0"></a></td>';                
                echo '<td>'.str_pad($r['sales_return_header_id'],7,0,STR_PAD_LEFT).'</td>';
    ...
    <?php else: #end else?>
        <div class="module_actions">
            ...
            <input type="hidden" name="sales_return_header_id" id="sales_return_header_id" value="<?=$aVal['sales_return_header_id']?>" />

            <fieldset style="border:none;">
                <p>                
                    <?php if( !empty($aVal['sales_return_header_id']) ){ ?>
                    <div>
                        SR#: <br>
                        <span style='font-weight:bold; font-size:14px;'><?=str_pad($aVal['sales_return_header_id'], 7,0,STR_PAD_LEFT)?></span>
                    </div>
                    <?php } ?>
        ...
        <?php if( $aVal['sales_return_header_id'] ){ 
        /*$aVal['status'] = "S";*/
        ...
        <?php if( $aVal['sales_return_header_id'] ): ?>
        <div>
            <?=displayDetails($aVal['sales_return_header_id']);?>    
        </div>
        <?php endif; ?>
        <?php if($_REQUEST['b'] == "Print Preview" && $aVal['sales_return_header_id']){ 
            echo " <iframe id='JOframe' name='JOframe' frameborder='0' src='print_sales_return.php?id=$aVal[sales_return_header_id]' width='100%' height='500'>
            </iframe>";    
        } 
        ?>
    <?php endif; #end if ?>    
