...
<div class=form_layout>
    ...
    <div class="module_actions">       
		...
		<div class="inline">
            Chart of Account<br />
            <?=$options->option_all_chart_of_accounts($_REQUEST['gchart_id'])?>
        </div>
    ...
    <div style="padding:3px; text-align:center;" id="content">
     ...
     if(!empty($_REQUEST['from']) && !empty($_REQUEST['to']))
		{
	?>
   		<iframe id="JOframe" name="JOframe" frameborder="0" src="print_cv_history.php?from=<?=$_REQUEST['from']?>&to=<?=$_REQUEST['to']?>&gchart_id=<?=$_REQUEST['gchart_id']?>" width="100%" height="500">
        </iframe>
    </div>
    <?php }?>
    </div>
