...
$include_opt = $_REQUEST['include_opt'];
?>
...
<div class=form_layout>
    ...
    <div class="module_actions">       
		...
		<div class="inline">
            Parent Account<br />
            <?=$options->option_parent_chart_of_accounts($_REQUEST['gchart_id'])?>
    ...
    <div style="padding:3px; text-align:center;" id="content">
     <?php
     if(!empty($_REQUEST['from']) && !empty($_REQUEST['to']) && !empty($_REQUEST['gchart_id']))
		{
	?>
   		<iframe id="JOframe" name="JOframe" frameborder="0" src="print_parent_account_report.php?from=<?=$_REQUEST['from']?>&to=<?=$_REQUEST['to']?>&gchart_id=<?=$_REQUEST['gchart_id']?>&include_opt=<?=$_REQUEST['include_opt']?>" width="100%" height="500">
        </iframe>
    </div>
    <?php }?>
    </div>
