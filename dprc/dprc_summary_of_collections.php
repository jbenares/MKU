...
<div class=form_layout>
    ...
    <div class="module_actions">       
        ...
        <div style="display:inline-block;">
            Post Code<br />
			<?=$options->getTableAssoc($_REQUEST['postcode'],'postcode','Select Post Code',"select * from dprc_post_codes order by postcode_desc",'postcode','postcode_desc')?>
        </div>
    ...
    <div style="padding:3px; text-align:center;" id="content">
     <?php
		...
		{
	?>
   		<iframe id="JOframe" name="JOframe" frameborder="0" src="dprc/print_dprc_summary_of_collections.php?from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>&postcode=<?=$_REQUEST['postcode']?>" width="100%" height="500">
        </iframe>
    </div>
    <?php }?>
    </div>
