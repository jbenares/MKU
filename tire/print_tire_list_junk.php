			...
			<?php
				if(!empty($type_id)){
					...
					while($rr=mysql_fetch_assoc($sql)){
									...
									$query="SELECT * FROM productmaster as p,junk_tires as j
												WHERE tire_type = '$rr[type_id]'
											 AND
												categ_id1 = '10'
											 AND
												categ_id2 = '30'
											 AND
												branding_number !=''
											 AND
												p.branding_number = j.branding_num
										     ORDER BY
												branding_number ASC
											";
									$result=mysql_query($query) or die(mysql_error());
