<?php
	...
	function getStocks($from_date,$to_date,$supplier,$rr_type,$po_header_id,$categ_id,$stock_id,$work_category_id,$sub_work_category_id,$project_id){
		$query="
			select
			...
			and
				rr_type = '$rr_type'
		";
		if($po_header_id) $query.=" and h.po_header_id = '$po_header_id' ";	
		if($categ_id1){
		$query.="
			and
     ...
     <div><!--Start of Form-->
        ...
        <div class="content" style="">
        	<table cellpadding="3">
				...
				foreach( getStocks($from_date,$to_date,$supplier,$rr_type,$po_header_id,$categ_id,$stock_id,$work_category_id,$sub_work_category_id,$project_id) as $iStock_id):
				$stock_id = $iStock_id;
				?>
             	...
             	<?php
					...
					$t_quantity = 0;
					$t_amount = 0;
					$t_optional_quantity = 0;
					$result=mysql_query($query) or die(mysql_error());
					$hasResults = mysql_num_rows($result) > 0;
					while($r=mysql_fetch_assoc($result)){
						...
						$t_optional_quantity 	+= $r['quantity_cum'];
						$t_amount 				+= $r['amount'];
                ...
                <?php if($hasResults): ?>
                	<tr class="borderTop">
                        ...
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td style="text-align:right; font-weight:bold; border-bottom:1px solid #000;"><?=number_format($t_quantity,2,'.',',')?></td>
                        <td>&nbsp;</td>
                        <td style="text-align:right;"><span style='border-bottom:1px solid #000; font-weight:bold;'><?=number_format($t_optional_quantity,2,'.',',')?></span></td>
                        <td>&nbsp;</td>
                        <td style="text-align:right; font-weight:bold; border-bottom:1px solid #000;"><?=number_format($t_amount,2,'.',',')?></td>
                    </tr>
                    <tr>
                    	<td>&nbsp;</td>
                    </tr>
                ...
                <tr>
                    ...
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td style="text-align:right; font-weight:bold; border-bottom:3px double #000;"><?=number_format($total_quantity,2,'.',',')?></td>
                    <td>&nbsp;</td>
                    <td style="text-align:right;"><span style='border-bottom:3px double #000; font-weight:bold;'><?=number_format($total_optional_quantity,2,'.',',')?></span></td>
                    <td>&nbsp;</td>
                    <td style="text-align:right; font-weight:bold; border-bottom:3px double #000;"><?=number_format($total_amount,2,'.',',')?></td>
                </tr>
            </table>
