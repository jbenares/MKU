...
<div class=form_layout>
    ...
    <div style="padding:3px; text-align:center;" id="content">
     <?php	if($from_date && $to_date && $project_id && $b=="Generate Report"){ ?>
   		<iframe id="JOframe" name="JOframe" frameborder="0" src="print_aggregates_issuance_history.php?from_date=<?=$from_date?>&to_date=<?=$to_date?>&project_id=<?=$project_id?>" width="100%" height="500">
        </iframe>
    <?php } else if( $from_date && $to_date && $b=="View All Projects" ) { ?>
    	<iframe id="JOframe" name="JOframe" frameborder="0" src="print_aggregates_issuance_history_all_projects.php?from_date=<?=$from_date?>&to_date=<?=$to_date?>" width="100%" height="500">
        </iframe>
    <?php } else if( $from_date && $to_date && $b=="View Summary" ) { ?>
    	<iframe id="JOframe" name="JOframe" frameborder="0" src="print_aggregates_issuance_history_summary.php?from_date=<?=$from_date?>&to_date=<?=$to_date?>" width="100%" height="500">
        </iframe>
    <?php } ?>
