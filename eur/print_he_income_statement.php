...
function getIncomeStatement($from_date,$to_date){
	...
	$result = mysql_query("
		select 
			eq_cat_name, sum(h.rate_per_hour * computed_time) as income
		from
		...
		and h.`status` != 'C'
		and d.released_date between '$from_date' and '$to_date'
		group by eq.eq_catID
		order by eq_cat_name asc
	") or die(mysql_error());
	$t = array();
	while($r = mysql_fetch_assoc($result)){
		$t['eq_cat_name'] = $r['eq_cat_name'];
		$t['amount'] = $r['income'];
	...
	$result = mysql_query("
		select 
			sum(amount) as amount, eq_cat_name
		from
