...
class PS_Pagination {
	...
	function renderFirst($view) {
		...
		if($this->page == 1) {
			...
			return '<a href="'.$this->php_self.'?view='.$view.'&page=1">'.$tag.'</a>';
		}
	}
	...
	function renderLast($view) {
		...
		if($this->page == $this->max_pages) {
			...
			return '<a href="'.$this->php_self.'?view='.$view.'&page='.$this->max_pages.'">'.$tag.'</a>';
		}
	}
	...
	function renderNext($view) {
		...
		if($this->page < $this->max_pages) {
			return '<a href="'.$this->php_self.'?view='.$view.'&page='.($this->page+1).'">'.$tag.'</a>';
		}
		else {
			return $tag;
		}
	...
	function renderNav($view) {
		...
		for( $i=$start ; $i<$end ; $i++) {
			if($i == $this->page) {
				...
				$links .= ' <a href="'.$this->php_self.'?view='.$view.'&page='.$i.'">'.$i.'</a> ';
			}
		}
	...
	function renderFullNav($view) {
		return $this->renderFirst($view).'&nbsp;'.$this->renderPrev($view).'&nbsp;'.$this->renderNav($view).'&nbsp;'.$this->renderNext($view).'&nbsp;'.$this->renderLast($view);	
	}
