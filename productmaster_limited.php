	  ...
	  if(!empty($checkList)) {
		foreach($checkList as $ch) {
			...
			if(	$options->stockIsPresent('budget_detail',$ch) || 
				...
				$msg ="Deleted";
				#mysql_query("delete from productmaster where stock_id='$ch'");
			}
		}
	  }
	}
...
<form name="_form" id="newareaform" action="" method="post">
<div class=form_layout>
	<div class="module_title"><img src='images/user_orange.png'><?=$transac->getMname($view);?></div>
    <div class="module_actions">
    	<input type="text" name="keyword" class="textbox" value="<?=$keyword;?>" placeholder="Stock"/>
		<input type="text" name="barcode" class="textbox" value="<?=$barcode?>" placeholder="OEM #" />
    ...
    <table cellspacing="2" cellpadding="5" width="100%" align="center" class="search_table">
        ...
        <tr>
            <td colspan="5" align="left">
            	<?php
					...
					#$pagination .= (!empty($_REQUEST['stockcode'])) ? "&stockcode=".$_REQUEST['stockcode']: "";
					$pagination .= ( $barcode ) ? "&barcode=".$barcode : "";
                ?>
                <?php
		...
		<?php								
			while($r=mysql_fetch_assoc($rs)) {
				...
				echo '<td align="left">'.$r[barcode].'</td>';	
