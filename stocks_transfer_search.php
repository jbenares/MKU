...
<?php
	...
	$transfer_header_id_pad	= str_pad($transfer_header_id,7,0,STR_PAD_LEFT);
		...
		foreach($checkList as $ch) {	
			...
			$query="
				...
				set
					status='C'
				where
					transfer_header_id = '$ch'
			";
			mysql_query($query);
			$options->insertAudit($ch,'transfer_header_id','C');
...
<div class=form_layout>
    ...
    <div class="module_actions">       
        <div style="display:inline-block;">
            ...
            <select class="select" name="field">
                	<option value="transfer_header_id" <?php if($field=="transfer_header_id") echo "selected='selected'"; ?>>Transfer #</option>
    ...
    <table id="my_table" cellspacing="2" cellpadding="5" width="100%" align="center" class="search_table">
    	<?php
			...
			$sql = "
				select
					*
				from
					transfer_header as h, projects as p
				where
		...
		while($r=mysql_fetch_assoc($rs)) {
			$transfer_header_id	= $r['transfer_header_id'];
			$transfer_header_id_pad	= str_pad($transfer_header_id,7,0,STR_PAD_LEFT);
		...
		?>
            ...
            <td width="15"><a href="admin.php?view=9584cf38e569679c0b52&transfer_header_id=<?=$transfer_header_id?>" title="Show Details"><img src="images/edit.gif" border="0"></a></td>
            <td width="15"><a href="admin.php?view=<?=$view?>&transfer_header_id=<?=$transfer_header_id?>&b=Print" title="Print Preview"><img src="images/action_print.gif" border="0"></a></td>
            <td><?=$transfer_header_id_pad?></td>
    ...
    ?>
    	<iframe id='JOframe' name='JOframe' frameborder='0' src='print_report_transmittal.php?id=<?=$transfer_header_id?>' width='100%' height='500'>
       	</iframe>
    <?php
	}
    ?>
