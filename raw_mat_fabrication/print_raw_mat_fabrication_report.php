...
define('TITLE', "RAW MATERIALS FABRICATION SUMMARY");
...
function getReport($from_date,$to_date, $to_project_id ){
  ...
  $sql="
    ...
    from
      fabrication as f 
      inner join fabrication_raw_mat as d on f.fabrication_id = d.fabrication_id
      inner join productmaster as p on d.raw_mat_stock_id = p.stock_id
  ...
  while( $r = mysql_fetch_assoc( $result ) ){
    ...
    $a['summary']['t_quantity']     += $r['raw_mat_quantity'];
    $a['summary']['t_total_weight'] += $r['raw_mat_total_weight'];
                      ...
                      foreach( $aReport['details'] as $r ){
                        echo "
                            <tr>
                              ...
                              <td>".number_format($r['raw_mat_weight_per_unit'],4)."</td>
                              <td>".number_format($r['raw_mat_quantity'],4)."</td>
                              <td>".number_format($r['raw_mat_total_weight'],4)."</td>
                            </tr>
                        ";    
                    }               
                    ?>
