...
define('TITLE', "WASTE CUT FABRICATION SUMMARY");
...
function getReport($from_date,$to_date, $to_project_id ){
  ...
  $sql="
    select
      f.*, stock, to_proj.project_name as to_project_name,date, stock_length
    from
      fabrication as f       
      left join productmaster as p on f.excess_stock_id = p.stock_id
  ...
  while( $r = mysql_fetch_assoc( $result ) ){
    $a['summary']['t_quantity']     += $r['excess_quantity'];
    $a['summary']['t_total_weight'] += $r['excess_total_weight'];
                      ...
                      foreach( $aReport['details'] as $r ){
                        echo "
                            <tr>
                              ...
                              <td>$r[stock]</td>
                              <td>$r[excess_length]</td>
                              <td>".number_format($r['excess_weight_per_unit'],4)."</td>
                              <td>".number_format($r['excess_quantity'],4)."</td>
                              <td>".number_format($r['excess_total_weight'],4)."</td>
                            </tr>
                        ";    
                    }               
                    ?>
