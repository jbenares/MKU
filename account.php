...
if($b=='Delete Selected') {
  if(!empty($checkList)) {
	foreach($checkList as $ch) {	
		mysql_query("delete from account where account_id='$ch'") or die (mysql_error());
	}
  }
}
?>
...
<div class=form_layout>
    ...
    <div class="module_actions">
        ...
        <input type="submit" name="b" value="Search Account" class="buttons" />
        <input type="button" name="b" value="Add Account" onclick="xajax_new_accountform();" class="buttons" />
    ...
    <table cellspacing="2" cellpadding="5" width="100%" align="center" class="search_table" style="text-align:left;">
    	<?php
			...
			$sql = "
				select
					*
				from
					account as a, account_type as t
				where
					a.account_type_id = t.account_type_id
				and
    	...
    	<tr bgcolor="#C0C0C0">				
          ...
          <td width="20"></td>
          <td width="80"><b>Account Code</b></td>
          <td><b>Account</b></td>
          <td width="300"><b>Account Type</b></td>
		...
		<?php								
			while($r=mysql_fetch_assoc($rs)) {
				...
				echo '<td width="15"><a href="#" onclick="xajax_edit_accountform(\''.$r[account_id].'\');" title="Edit Entry"><img src="images/edit.gif" border="0"></a></td>';
				echo '<td>'.$r[account_code].'</td>';	
