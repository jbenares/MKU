        ...
        ?>
            <div id="accordion">
                ...
                <div>
                    ...
                    <tr>
                        <td><?=$i++?></td>
                        <td><input type="button" value="REQUEST" onclick="xajax_pr_service_stock_id_form('<?=$stock_id?>');" /></td>
                        <td><?=$stock?></td>
...
<div class="results">
    ...
    if($rows > 0):
    ?>    
        <div style="background-color::#FFF; font-weight:bolder; margin-top:10px;">PURCHASE REQUEST SERVICE DETAILS : </div>
        <table cellspacing="2" cellpadding="5" width="100%" align="center" class="display_table" id="search_table" style="border:1px solid #000;">
            ...
            ?>
                <tr>
                    <td><?=$i++?></td>
                    <td><a href="admin.php?view=<?=$view?>&pr_header_id=<?=$pr_header_id?>&id=<?=$pr_service_detail_id?>&b=S" ><img src="images/trash.gif" onclick="return approve_confirm();" /></a></td>
                    <td><?=$stock?></td>
