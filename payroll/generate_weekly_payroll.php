...
function getEmployeePayrollAttributes($employeeID,$payroll_sequence_id){
	...
	/*compute pag-ibig*/
	$aReturn['pagibig']    = ( $payroll_sequence_id == 4 ) ? 100 : 0;
	...
	if( $computed_salary > 50 ) {
		...
		$getPH = mysql_query("
			...
			from
				philhealth_contrib
			where
				ph_range_min <= '$computed_salary' 
			and ph_range_max >= '$computed_salary' 
			and phvoid = '1'
		") or die(mysql_error());
		
		
		if(mysql_num_rows($getPH) == 0) { /*no rows found*/			
			$philhealth = 0;
			return  $philhealth;							
		}					
		...
		if( $payroll_sequence == '4' ){			
			$philhealth = $rPH['max_ee'];			
		} else {			
			$philhealth = 0.00;
		}

	} else { /*if salary is less than 50 pesos, he/she has no philhealth*/
		$philhealth = 0;
	}	
...
function computeSSS($computed_salary,$payroll_sequence){
	...
	$rSSS = mysql_fetch_assoc($getSSS);				 								
	
	if(mysql_num_rows($getSSS) == 0) { /*computed salary not within range*/
		$sss = 0;
		return $sss;								
	}					
	
	if($payroll_sequence == '4'){ /*if last payroll sequence is then get deduction*/
		$sss = $rSSS['ee'];			
	} else {
		$sss = 0.00;	
	} 
...
function getEmployees($companyID,$project_id,$employee_type_id,$release_type_id = 2){
	$sql = "
		select 
			* 
		from 
...
<div class="module_actions">	
	<table>
        ...
        <tr>
        	...
        	<td><?=lib::getArraySelect($payroll_sequence_id,'payroll_sequence_id',"Select Payroll", $arr)?></td>        	
