...
function getLateOrAbsent($date,$employeeID){
	...
	if( mysql_num_rows($result) > 0 ){
		...
		if( $dtr_time_in != "00:00:00" ){
			if( $emp_time_in == "07:30:00" ){ //for Time-in, not actual
				
				//1st parsed
				$parsed = date_parse($dtr_time_in);
		
				//2nd parsed
				$parsed2 = date_parse($emp_time_in); // employee in assigned
				...
				if( $time_in < "07:36:00" ){						
					...
					$aReturn['late'] = 60;
				} else if( $time_in >= "08:01:00" && $time_in <= "11:00:00" ){
					$aReturn['late'] = 240;
				} else if( $time_in >= "11:00:00" ){
					$aReturn['late'] = 0;
				}
			} else if( $emp_time_in == "08:00:00" ){ //for Time-in, not actual
				...
				if( $time_in < "08:06:00" ){						
					...
					$aReturn['late'] = 60;
				} else if( $time_in >= "09:01:00" && $time_in <= "11:00:00" ){
					$aReturn['late'] = 240;
				} else if( $time_in >= "11:00:00" ){
					$aReturn['late'] = 0;
				}
						...
						foreach( $aReport as $r ){
		                    if(count($r['dtr']) > 0){
		                    	...
		                    	foreach( $r['dtr'] as $arr_dtr ){
		                    		if( $i == 0 ){
										$total_min += $arr_dtr[late];
										$total_abs += $arr_dtr[absences];
										$total_sec += $arr_dtr['sec'];
										$total_hr += $arr_dtr['hr'];
										$total_minute += $arr_dtr['min'];									
										...
										if($arr_dtr[dtr_time_in] >= '11:00:00'){
											$arr_dtr['hr'] = 0;
											$arr_dtr['min'] = 0;
											$arr_dtr['sec'] = 0; 
											$arr_dtr[dtr_time_in] = '';
										...
										if($arr_dtr[dtr_time_in] < '11:00:00' && !empty($arr_dtr[ob])){
											$arr_dtr['hr'] = 0;
											$arr_dtr['min'] = 0;
											$arr_dtr['sec'] = 0;
											$arr_dtr[late] = 0;
									...
									$hr1  = 0;	
									//seconds
									 $sec1 = floor($total_sec/60); //additional minutes					
									 $sec2 = $total_sec - ($sec1 * 60);  // seconds
									 
									//minutes
									 $min1 = floor($total_minute/60); //additional hour
									 $min2 = ($total_minute - ($min1 * 60)) + $sec1;  // total minutes, remainder from seconds added
									 $min3 = 0; //var min
									
									 if($min2 > 60){
										$min3 = $min2 - 60; //minutes
										$min2 = 1; // additional hour		
									//hours
										$hr1 = $total_hr + $min1 + $min2;
									}else{
										$hr1 = $total_hr + $min1;	
									}
										...
										echo "
											<tr>
												...
												<td style='border-top: 1px solid #000;text-align: center;' >".$total_abs."</td>
												<td style='border-top: 1px solid #000;'>".$total_min."(".$total_min/60 ." hrs)</td>
												<td style='border-top: 1px solid #000;'>".$hr1.':'.str_pad($min2,2,"0",STR_PAD_LEFT).':'.str_pad($sec2,2,"0",STR_PAD_LEFT)."</td>
