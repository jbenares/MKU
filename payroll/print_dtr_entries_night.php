...
<?php
	...
	function getTimeIn($e_id,$day){
		$sql = mysql_query("select 
				...
				be.empID = '$e_id' and
				be.b_date = '$day' and
				be.b_time between '12:00:00' and '24:00:00'") or die (mysql_error());
	...
	$otd = date('Y-m-d', strtotime($day. ' + 1 days'));		
		...
		$sql = mysql_query("select 
				...
				be.empID = '$e_id' and
				be.b_date = '$otd' and
				be.b_time between '00:00:00' and '11:59:59'") or die (mysql_error());
	...
	<?php
		...
		$emp_time_in = '17:06:00';		
		$emp_time_out = '01:00:00';		
		while($r = mysql_fetch_assoc($result)){
			$hrs_worked = 0;
			$overtime = 0;
			...
			if((!empty($time_in) && $time_in <= $emp_time_in) && (!empty($time_out) && $time_out >= $emp_time_out)){
				...
				if(!empty($time_in) && !empty($time_out)){
					...
					if($time_in >= '17:06:01' && $time_in <= '17:16:00'){
						$late = .5;
										
					}else if($time_in >= '17:16:01' && $time_in <= '17:30:00'){
						$late = 1;
						
					}else if($time_in > '17:30:00'){
						$late = 4;
					}
					
					if($time_out < '00:30:00'){
						$late2 = 4;
						
					}else if($time_out >= '00:30:00' && $time_out <= '00:44:00'){
						$late2 = 1;
					}else if($time_out < '00:54:00' && $time_out >= '00:44:01'){
						$late2 = .5;
					}	
