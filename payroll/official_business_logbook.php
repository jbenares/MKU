...
$aHeader 	= 	array(
					'employee_id','from_project_id','to_project_id','notes','date','time'
				);
...
if($aVal['b'] == "Submit"){
	if(empty($aVal['official_logbook_id'])){
		...
		mysql_query("
			update official_logbook set user_id = '$user_id',time = NOW() where official_logbook_id = '$aVal[official_logbook_id]'
		") or die(mysql_error());
...
<form enctype='multipart/form-data' method="post" action="" id="newareaform" >
	...
	<?php if($aVal['b'] == "Search"): ?>
        ...
        $sql = "
            select
				concat(e.employee_lname,', ',e.employee_fname) as employee, official_logbook_id, date, time, f.project_name as from_project, t.project_name as to_project, notes
			from
        ...
        <div class="module_actions">
        	<div style="display:inline-block;">
             ...
             <div style="display:inline-block; margin-left:10px;">
            	<em>Encoded:</em> <br />
            	<span style="font-weight:bold;"><?=$aVal['date']?> <?=$aVal['time']?></span>
            </div>           
            <div style="display:inline-block; margin-left:10px;">
            	<em>Encoded by:</em> <br />
