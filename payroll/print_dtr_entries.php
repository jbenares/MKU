	...
	if(!empty($fdate) && !empty($tdate)) {
		...
		echo '<table width="100%" border="0" cellspacing="0" cellpadding="3">';
			...
			echo '<tr>
				    ...
				    <td class=line_bottom><b>In</b></td>
				    <td class=line_bottom><b>Out</b></td>
				    <td class=line_bottom><b>Out(AM)</b></td>
				    <td class=line_bottom><b>Hrs Worked</b></td>
				    <td class=line_bottom><b>OT(HR)</b></td>
					<td class=line_bottom><b>OB#</b></td>
					<td class=line_bottom><b>AA</b></td>
				</tr>';
			...
			if($dtrb_date[$main_index-1]!=$r[b_date] || $dtrempID[$main_index-1]!=$r[employeeID]) {
				...
				$sql_rw = mysql_query("select
							...
							where
								...
								b_date='$r[b_date]' and
								b_time between '06:00' and '11:00'");
				...
				//check for next day out	
				if(empty($dtrout2[$main_index])){
					
				$otd = date('Y-m-d', strtotime($r['b_date']. ' + 1 days'));	
				...
				$sql_rw = mysql_query("select
							...
							where
								empID='$r[employeeID]' and
								b_date='$otd' and
								b_time between '00:00' and '06:00'");
								
								
				$rw = mysql_fetch_array($sql_rw);
				$dtrout_mot[$main_index] = $rw['outpm'];		
		...
		foreach ($dtrempID as $key => $demp) {
			...
			$out_mot = $dtrout_mot[$key];
			$out_pm_ot = $dtrout2[$key];
			...
			$hr = 0;
			//overtime - am
			if($rti[emp_time_in]=="07:30:00" && ($out_mot>="00:00" && $out_mot<="06:00")) {
				
				$time = "07:30:00";
				$parsed = date_parse($out_mot);
				$parsed2 = date_parse($time);	

				$hr =  $parsed['hour'] + $parsed2['hour'];
				$min = $parsed['minute'] + $parsed2['minute'];
				
				if($min >= 60){
				$hr += 1;	
				}
				
			}else if($rti[emp_time_in]=="08:00:00" && ($out_mot>="00:00" && $out_mot<="06:00")) {
			...
			//overtime - pm
			if($rti[emp_time_in]=="07:30:00" && ($out_pm_ot>="16:30" && $out_mot<="24:00")) {
				
				$time = "16:30:00";
				$parsed2 = date_parse($time);
				$parsed = date_parse($out_pm_ot);
				if($parsed['hour'] > $parsed2['hour']){
					
					$partial_min = ((($parsed['hour'] * 60) + $parsed['minute']) - (($parsed2['hour'] * 60) + $parsed2['minute']))/60;
					$hr =  floor($partial_min);
				}
				
			}else if($rti[emp_time_in]=="08:00:00" && ($out_pm_ot>="17:00" && $out_mot<="24:00")) {
			...
			if($mins_late >= 16 && $mins_late < 31) $late = 1;
			if($mins_late >= 31) $late = 4;
			...
			if($demp!=$dtrempID[$key+1]) {
				echo '<tr>
		   	        	<td colspan=10 style="border-top:1px solid #000000;text-align:right;">Total :</td>
			        	<td style="border-top:1px solid #000000;">'.$totalhrs.' Hrs</td>
			        	<td style="border-top:1px solid #000000;">'.$total_ot.' Hrs</td>
