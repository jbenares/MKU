	...
	*/
	...
	class options {
		...
		function getUserName($userID){
			...
			return "$r[user_lname], $r[user_fname]";
		}
    ...
    function getTirePosition($selectedid=NULL,$name='position'){
		...
		{
			$type=array('1'=>'BDO', '2'=>'CHINA BANK', '3' => 'METROBANK' );
			$content="
		...
		***********************************/
		...
		function getAPBalanceOnAndOverDate($supplier_id,$date){
			$payables = $this->getBalanceFromSupplier($date,$supplier_id);
			$payments	= $this->getDisbursementForSupplier($date,$supplier_id);

			return $payables - $payments;
		}


		function getBalanceFromSupplier($date,$supplier_id){
			# MATERIALS RECEIVED
			$result=mysql_query("
						select
							sum(netamount) as total_amount
		...
		function getDisbursementForSupplier($date,$supplier_id){
		...
		function postIssuance($issuance_header_id,$contractor_id = NULL){
			...
			*/
			...
			#	13	 	- DIRECT MATERIALS		DEBIT
			#	703 	- MATERIALS INVENTORY				CREDIT
			...
			$inventory_account = 703;
			$direct_materials_account = 13;
		...
		function postRR($rr_header_id){
			...
			*/
			...
			if($rr_type == "A"){
				if(!empty($ppe_gchart_id)){
					...
					$this->insertIntoGLDetails($gltran_header_id,$ppe_gchart_id,$total_amount,0,$project_id);
				}else{
					//$this->insertIntoGLDetails($gltran_header_id,$ppe,$total_amount-$discount_amount,0,$project_id);
					$this->insertIntoGLDetails($gltran_header_id,$ppe,$total_amount,0,$project_id);
				}
			}else{
				#$this->insertIntoGLDetails($gltran_header_id,$inventory_account,$total_amount-$discount_amount,0,$project_id);
                $this->insertIntoGLDetails($gltran_header_id,$inventory_account,$total_amount,0,$project_id);
			}
			...
			*/
            $this->insertIntoGLDetails($gltran_header_id,$payable_to_suppliers,0,$total_amount,$project_id);
		...
		function postServicePO($po_header_id,$account_id){
			...
			*/
			...
			/*
				insert inventory
			*/
			$labor_expense = $account_id;
			$accounts_payable  = 729;
			$this->insertIntoGLDetails($gltran_header_id,$labor_expense,$total_amount,0);
			$this->insertIntoGLDetails($gltran_header_id,$accounts_payable,0,$total_amount);
		...
		function postCV($cv_header_id){
			...
			if($type == "M"){
                ...
                if($amount > 0){
					$this->insertIntoGLDetails($gltran_header_id,$ap_gchart_id,$amount+$discount_ap,0);
                }
		...
		function inventory_warehouse($date,$stock_id,$quantity_type = "quantity"){
			...
			*/
			...
			if($quantity_type == "quantity"){
				...
				$issuance_qty 	= $this->inventory_issuance($date,$stock_id,9);	#has cum
				$transfer_qty	= $this->inventory_stockstransfer($date,$stock_id,9);
				$returns_qty	= $this->inventory_stocksreturn($date,$stock_id);
				$adjustments	= $this->inventory_adjustments($date,$stock_id,9);
				...
				$issuance_qty 	= $this->inventory_issuance($date,$stock_id,9,"quantity_cum");	#has cum
			}
