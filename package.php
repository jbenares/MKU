	...
	if($b=='Delete Selected') {
	  if(!empty($checkList)) {
		foreach($checkList as $ch) {	
			mysql_query("delete from package where package_id='$ch'");
		}
	  }
	}
?>
...
<div class=form_layout>
    ...
    <div class="module_actions">
        ...
        <input type="submit" name="b" value="Search Package" class="buttons" />
      <!--  <input type="submit" name="b" value="Display All" class="buttons" />-->
        <input type="button" name="b" value="Add Package" onclick="xajax_new_packageform();toggleBox('demodiv',1);" class="buttons" />
    ...
    <table cellspacing="2" cellpadding="5" width="100%" align="center" class="search_table">
		...
		<?php								
			while($r=mysql_fetch_assoc($rs)) {
				...
				echo '<td><input type="checkbox" name="checkList[]" value="'.$r[package_id].'" onclick="document._form.checkAll.checked=false"></td>';
				echo '<td width="15"><a href="#" onclick="xajax_edit_packageform(\''.$r[package_id].'\');" title="Edit Entry"><img src="images/edit.gif" border="0"></a></td>';
				echo '<td>'.$r[packagecode].'</td>';	
